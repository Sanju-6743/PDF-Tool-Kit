...
def register_routes(app, socketio):
    ...
    def merge_route():
        if 'files[]' not in request.files:
            ...
            return jsonify({'status': 'error', 'message': 'No PDF files were uploaded.'})
        ...
        if len(files) < 2:
            ...
            return jsonify({'status': 'error', 'message': 'At least 2 PDF files are required for merging.'})
        ...
        if len(file_paths) < 2:
            ...
            return jsonify({'status': 'error', 'message': 'At least 2 valid PDF files are required for merging.'})
        ...
        return jsonify({'status': 'processing', 'message': 'Merging PDFs...'})
